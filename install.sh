...
getAppVersion(){
    version=$(sudo curl -Ls "https://github.com/arian33x0/rocket-ssh/releases/tag/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
    echo $version;
}
...
installationInfo(){
    clear
    echo -e "\n"
    bannerText=$(curl -s https://raw.githubusercontent.com/arian33x0/rocket-ssh/master/rocket-banner.txt)
    printf "%s" "$bannerText"
    echo -e "\n"
...
nethogsLink=https://raw.githubusercontent.com/HamedAp/Nethogs-Json/main/install.sh
